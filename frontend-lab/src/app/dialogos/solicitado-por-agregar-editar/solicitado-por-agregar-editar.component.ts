  ...
  agregarEditarSolicitadoPor(){
    ...
    if(this.dataSolicitadoPor == null){
      ...
      if (this.dataSolicitadoPor.id !== undefined) {
        this.solicitadoPorService.modificarSolicitadoPor(this.dataSolicitadoPor.id, modelo).subscribe({
          next: (data) => {
            this.mostrarAlerta("Area solicitante editada correctamente.", "Listo");
            this.dialogoReferencia.close({solicitadoPor: data});
          },
          error: (e) => {
            this.mostrarAlerta("No se pudo editar el area solicitante, revise los campos", "Error");
          }
        });
      } else {
    ...
    this.solicitadoPorService.verificarSolicitadoPorExistente(id).subscribe({
      next: (existeSolicitadoPor: boolean) => {
        if (existeSolicitadoPor) {
          this.mostrarAlerta('Ya hay un area solicitante con esa nombre', 'Error');
        } else {
