...
export class NuevaOrdenAgregarEditarComponent implements OnInit {
  ...
  constructor(
    ...
    this.formNuevaOrden = this.fb.group({
      numeroDocumento: ['',
        [Validators.required,
        Validators.pattern(/^[0-9A-Za-z]{7,9}$/
        )]],
      apellido: ['',
      [Validators.required]],
      nombre: ['',
  ...
  ngOnInit() {
    this.nuevaOrdenService.obtenerListadoAnalisis().subscribe(data => {
      ...
      this.listadoAnalisisValores = data.sort((a: { nombre: string; }, b: { nombre: any; }) => a.nombre.localeCompare(b.nombre));
  
    });
   
    this.nuevaOrdenService.obtenerListadoGruposAnalisis().subscribe(data => {
    
    this.listadoGruposValores = data.sort((a: { nombreDelGrupo: string; }, b: { nombreDelGrupo: any; }) => a.nombreDelGrupo.localeCompare(b.nombreDelGrupo));
    ...
    this.formNuevaOrden.get('numeroDocumento')?.valueChanges.subscribe((numeroDocumento: string) => {
      if (numeroDocumento) {
    ...
    const listadoAnalisisIds = this.dataNuevaOrden.analisis?.map((analisis: any) => analisis.id) || [];
    const listadoGruposIds = this.dataNuevaOrden.grupos_analisis?.map((grupo: any) => grupo.id) || [];
  ...
  obtenerListadoMedicos() {
    this.http.get<Medicos[]>('http://10.10.0.79:3000/medicos/listadoMedicos')
      .pipe(
        ...
        tap(medicos => {
          this.medicos = medicos.sort((a, b) => a.apellido.localeCompare(b.apellido));
          const medicoPreseleccionado = this.medicos.find(medico => medico.id === this.dataNuevaOrden.medicos?.id);
  ...
  compararAnalisis(analisis1: Analisis, analisis2: Analisis): boolean {
    return analisis1 && analisis2 ? analisis1.nombre === analisis2.nombre : analisis1 === analisis2;
  }

  compararGrupos(grupo1: GrupoAnalisis, grupo2: GrupoAnalisis): boolean {
    return grupo1 && grupo2 ? grupo1.nombreDelGrupo === grupo2.nombreDelGrupo : grupo1 === grupo2;
  }
  ...
  obtenerListadoLaboratoristas() {
    this.http.get<Laboratoristas[]>('http://10.10.0.79:300/laboratoristas/listadoLaboratoristas')
      .pipe(
        ...
        tap(laboratoristas => {
          this.laboratoristas = laboratoristas.sort((a, b) => a.apellido.localeCompare(b.apellido));
          const laboratoristaPreseleccionado = this.laboratoristas.find(laboratorista => laboratorista.id === this.dataNuevaOrden.laboratorista?.id);
          this.formNuevaOrden.patchValue({ laboratorista: laboratoristaPreseleccionado });
        })
      )
      .subscribe();
  }
  ...
  agregarEditarNuevaOrden() {
    ...
    const pacienteModelo = {
      id: this.dataNuevaOrden == null ? this.idPaciente : this.dataNuevaOrden.paciente.id, 
    ...
    if (this.dataNuevaOrden == null) {
      ...
      if (this.dataNuevaOrden.id !== undefined) {
        this.nuevaOrdenService.modificarOrden(this.dataNuevaOrden.id, modelo).subscribe({
          next: (data) => {
  ...
  CargarResultados(): void {
    ...
    this.dataNuevaOrden.analisis.forEach(analisis => {
      unidades[`analisis_${(analisis as unknown as Analisis).id}`] = (analisis as unknown as Analisis).unidades;
    });
    this.dataNuevaOrden.grupos_analisis.forEach((grupo: any) => {
      grupo.listado_de_analisis.forEach((analisis: Analisis) => {
        unidades[`analisis_${(analisis as Analisis).id}`] = (analisis as Analisis).unidades;
      });
    });
  ...
  agregarEditarLaboratorista() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result && result.laboratorista) {
        ...
        this.laboratoristas = this.laboratoristas.sort((a, b) => a.apellido.localeCompare(b.apellido));
        this.obtenerListadoLaboratoristas();
      }
    });
  }
  ...
  agregarEditarMedico() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result && result.medico) {
        ...
        this.medicos = this.medicos.sort((a, b) => a.apellido.localeCompare(b.apellido));
        this.obtenerListadoMedicos();
      }
    });
  }
