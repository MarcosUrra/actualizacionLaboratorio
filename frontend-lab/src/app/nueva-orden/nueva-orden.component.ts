...
export class NuevaOrdenComponent implements AfterViewInit, OnInit {
  ...
  filtrado(filtros: any) {
    this.nuevaOrdenService.filtrado(filtros).subscribe({
      next: (dataResponse) => {
        this.dataSource.data = dataResponse.map(
          (element: { fecha: string | number | Date | null }) => {
            element.fecha = this.datePipe.transform(
              element.fecha,
              'dd/MM/yyyy HH:mm:ss'
            );
            return element;
  ...
  DialogoUnaNuevaOrden() {
    this.dialog
      .open(NuevaOrdenAgregarEditarComponent, {
        disableClose: true,
        width: '80%',
      })
      .afterClosed()
  ...
  imprimirInforme(datos: any) {
    const dialogRef = this.dialog.open(SiNoResultadosComponent, {
      width: '250px',
      data: {
        mensaje: 'Una vez Impreso no se podra Modificar. ¿Confirma Impresion?',
      },
    });
      ...
      if (result === true) {
        ...
        const filasAnalisis = datos.analisis.map((analisis: any) => {
          const resultado = datos.resultados.find(
            (res: any) => res.id_analisis === analisis.id
          );
          return [
            { content: analisis.nombre },
            {
              content: resultado
                ? `${resultado.resultados} ${analisis.unidades}`
                : '',
            },
            {
              content: analisis.valores
                ? `${analisis.valores} ${analisis.unidades}`
                : '',
            },
          ];
        });

        const filasGruposAnalisis = datos.grupos_analisis.flatMap(
          (grupo: any) => {
            const nombreInforme = [
              {
                content: `${grupo.nombreInforme}`,
                styles: { fontStyle: 'bold' },
              },
            ];

            const analisisDelGrupo = grupo.listado_de_analisis.map(
              (analisis: any) => {
                const resultado = datos.resultados.find(
                  (res: any) => res.id_analisis === analisis.id
                );
        ...
        const laboratoristaNombre = datos.laboratorista
          ? datos.laboratorista.nombre
          : '';
        const laboratoristaApellido = datos.laboratorista
          ? datos.laboratorista.apellido
          : '';
        const laboratoristaMatricula = datos.laboratorista
          ? datos.laboratorista.matricula
          : '';
        ...
        doc.output('dataurlnewwindow', {
          filename: `Informe_${fechaFormateada}.pdf`,
        });
      }
    });
  }
  ...
  abrirCuadroDialogo(element: { _data: { _value: any[] } }) {
    ...
    dialogRef.afterClosed().subscribe((result: boolean) => {
      const incluirResultado =
        this.columnasSeleccionadas.includes('ListadoAnalisis');
      ...
      if (result === true) {
        ...
        this.srvImpresionCompleto.imprimir({
          tablaOrdenes: element._data._value,
          titulo: 'Listado de Ordenes',
          columnas: copiaColumnasSeleccionadas,
          guardar: true,
          filtroAnalisis: this.mostrarAnalisis,
          analisis: this.filtros.analisis,
        });
