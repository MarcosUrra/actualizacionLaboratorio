...
export class PacientesService {
  ...
  public async nuevoPaciente(nuevoPaciente: CreatePacienteDto): Promise<any> {
    ...
    if (errors.length > 0) {
      const errorMessage =
        'Error al intentar cargar un nuevo paciente: Por favor, complete todos los campos obligatorios';
      throw new HttpException(errorMessage, HttpStatus.BAD_REQUEST);
    }
    if (!nuevoPaciente.numeroDocumento) {
      const errorMessage =
        'Error al intentar cargar un nuevo paciente: hay campos obligatorios';
      throw new HttpException(errorMessage, HttpStatus.BAD_REQUEST);
    }
    ...
    if (existePaciente) {
      const errorMessage =
        'Error al intentar cargar un nuevo paciente: El paciente ya existe';
      throw new HttpException(errorMessage, HttpStatus.BAD_REQUEST);
    }
