...
export class UsuariosService {
    constructor(@InjectRepository(UsuariosEntity) private usuarioRepository:Repository<UsuariosEntity>){}
    ...
    async updateUsuario(id:number,usuario:UpdateUsuarioDto): Promise<UsuariosEntity | undefined>{
        const usuariofound= await this.usuarioRepository.findOne({
            where:{id},
        })
        if(!usuariofound){
    ...
    async validateUser(username: string, password: string, role:string): Promise<UsuariosEntity | null> {
