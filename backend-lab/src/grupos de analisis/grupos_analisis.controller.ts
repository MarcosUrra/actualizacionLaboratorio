...
export class GruposAnalisisController {
    ...
    obtenerGrupoPorNombre(@Param('nombreGrupo')nombreDelGrupo) {
        return this.gruposAnalisisService.obtenerGrupoAnalisisPorNombre(nombreDelGrupo);
    }
  
    @Post('/nuevoGrupo')
    nuevoGrupo(@Body() grupo: { nombreDelGrupo: string, listado_de_analisis: number[] }): Promise<GruposAnalisisEntity> {//GruposAnalisisEntity): Promise<GruposAnalisisEntity> {
