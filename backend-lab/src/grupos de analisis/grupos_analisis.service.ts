...
export class GruposAnalisisService {
  ...
  async nuevoGrupoAnalisis(body: any) {
    try {
      ...
      const analisis = await this.analisisRepository.findByIds(
        listado_de_analisis,
      );
  ...
  async modificarGrupoAnalisis(id, body: any) {
    let grupoActualizado = await this.gruposAnalisisRepository.findOne({
      where: { id },
    });
    ...
    const analisisAEliminar = (
      grupoActualizado.listado_de_analisis || []
    ).filter((x) => !analisis.includes(x));
