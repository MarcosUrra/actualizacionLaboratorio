...
export class LaboratoristasController {
  ...
  async crearLaboratorista(
    @Body(new ValidationPipe()) nuevoLaboratorista: crearLaboratoristaDto,
  ...
  @Patch(':id')
  modificarLaboratorista(
    @Param('id') id: number,
    @Body() laboratoristaDto: modificarLaboratoristaDto,
  ) {
    return this.laboratoristasService.modificarLaboratorista(
      id,
      laboratoristaDto,
    );
  }
