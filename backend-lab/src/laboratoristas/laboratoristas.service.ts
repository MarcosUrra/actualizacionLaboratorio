...
export class LaboratoristasService {
  ...
  async crearLaboratorista(
    nuevoLaboratorista: crearLaboratoristaDto,
  ): Promise<{ mensaje: string; laboratorista: LaboratoristasEntity }> {
    ...
    if (laboratoristaFound) {
      throw new HttpException(
        'Ya hay un laboratorista con esa matrícula',
        HttpStatus.BAD_REQUEST,
      );
    }

    const nuevoLaboratoristaEntity =
      this.laboratoristaRepository.create(nuevoLaboratorista);
    await this.laboratoristaRepository.save(nuevoLaboratoristaEntity);

    return {
      mensaje: 'Laboratorista guardado con éxito',
      laboratorista: nuevoLaboratoristaEntity,
    };
  }
  ...
  async modificarLaboratorista(
    id: number,
    laboratoristaDto: modificarLaboratoristaDto,
  ): Promise<LaboratoristasEntity | undefined> {
