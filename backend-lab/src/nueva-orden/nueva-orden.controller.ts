...
export class NuevaOrdenController {
  ...
  async ObtenerOrdenesPorFecha(
    ...
    if (!fechaDesde || fechaDesde.trim() === '' || !fechaHasta || fechaHasta.trim() === '') {
      const fechaActual = new Date();
      fechaDesde = `${fechaActual.getMonth() + 1}/${fechaActual.getDate() - 3}/${fechaActual.getFullYear()}`;
      fechaHasta = `${fechaActual.getMonth() + 1}/${fechaActual.getDate()}/${fechaActual.getFullYear()}`;
    }
    const [mesDesde, diaDesde, anioDesde] = fechaDesde.split('/');
    const [mesHasta, diaHasta, anioHasta] = fechaHasta.split('/');

    const fechaDesdeObj = new Date(Number(anioDesde), Number(mesDesde) - 1, Number(diaDesde), 0, 0, 0);
    const fechaHastaObj = new Date(Number(anioHasta), Number(mesHasta) - 1, Number(diaHasta), 23, 59, 59);

    try {
      const ordenes = await this.nuevaOrdenService.ObtenerOrdenesPorFecha(fechaDesdeObj, fechaHastaObj);
      return ordenes;
    } catch (error) {
      console.error('Error al obtener órdenes en el controlador:', error.message);
  ...
  async filtrado(
    ...
    try {
      const ordenes = await this.nuevaOrdenService.filtrado(fechaDesdeObj, fechaHastaObj, analisis, nroOrden, numeroDocumento, nombreCompleto);
      return ordenes;
    } catch (error) {
      console.error('Error al obtener órdenes en el controlador:', error.message);
      throw new Error('Error al obtener órdenes.');
  ...
  async ObtenerOrdenesReporteDiario(
    @Query('fecha') fecha: Date,
  ) {
    
    try {
      const ordenes = await this.nuevaOrdenService.ObtenerOrdenesReporteDiario(fecha);
      return ordenes;
    } catch (error) {
      console.error('Error al obtener órdenes en el controlador:', error.message);
      throw new Error('Error al obtener órdenes.');
