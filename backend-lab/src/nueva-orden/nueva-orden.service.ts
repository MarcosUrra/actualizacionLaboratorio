...
export class NuevaOrdenService {
  ...
  constructor(
    @InjectRepository(NuevaOrden) private nuevaOrdenRepository: Repository<NuevaOrden>,
    @InjectRepository(PacientesEntity) private pacienteRepository: Repository<PacientesEntity>,
    @InjectRepository(MedicosEntity) private medicoRepository: Repository<MedicosEntity>,
    @InjectRepository(LaboratoristasEntity) private laboratoristaRepository: Repository<LaboratoristasEntity>,
    @InjectRepository(AnalisisEntity) private analisisRepository: Repository<AnalisisEntity>,
    @InjectRepository(GruposAnalisisEntity) private gruposAnalisisRepository: Repository<GruposAnalisisEntity>,
  ) { }
  ...
  public async ObtenerOrdenes() {
    try {
      return await this.nuevaOrdenRepository
        ...
        .leftJoinAndSelect('GruposAnalisisEntity.listado_de_analisis', 'AnalisisGrupoEntity')
  ...
  public async ObtenerOrdenesPorFecha(
    fechaDesde: Date,
    fechaHasta: Date) {
    try {
      return await this.nuevaOrdenRepository
  ...
  public async filtrado(
    ...
    try {
      ...
      query.forEach((orden) => {
        const pacienteNombreCompleto = (orden.paciente.nombre + ' ' + orden.paciente.apellido).toLowerCase();
        ...
        if (
          (!analisis || analisisArray.some((a) => a.nombre.toLowerCase().includes(analisis.toLowerCase()))) &&
          (!nroOrden || (orden.id.toString().includes(nroOrden.toString())) || orden.id.toString().startsWith(nroOrden.toString())) &&
          (!numeroDocumento || orden.paciente.numeroDocumento.includes(numeroDocumento)) &&
          (!nombreCompleto || palabrasNombreCompleto.every(palabra => pacienteNombreCompleto.includes(palabra)))
        ) {

          resultadosFiltrados.push(orden);
        } else {
          resultadosFiltrados = resultadosFiltrados.filter(element => element.id !== orden.id);
        }
        if (!analisis && !nroOrden && !numeroDocumento && !nombreCompleto && fechaDesde && fechaHasta) {
  ...
  public async CrearOrden(nuevaOrden: EditarOrdenCreateDto): Promise<any> {
    ...
    const paciente = await this.pacienteRepository.findOne({ where: { id: nuevaOrden.paciente.id } });
    ...
    if (nuevaOrden.medicos && nuevaOrden.medicos.id) {
      medico = await this.medicoRepository.findOne({ where: { id: nuevaOrden.medicos.id } });
    }

    let laboratorista;
    if (nuevaOrden.laboratorista && nuevaOrden.laboratorista.id) {
      laboratorista = await this.laboratoristaRepository.findOne({ where: { id: nuevaOrden.laboratorista.id } });
    }
    ...
    nuevaOrdenModel.analisis = nuevaOrden.analisis ? analisisIds.map(id => ({ id: Number(id) } as AnalisisEntity)) : [];
    nuevaOrdenModel.grupos_analisis = nuevaOrden.grupos_analisis ? gruposIds.map(id => ({ id: Number(id) } as GruposAnalisisEntity)) : [];

    return await this.nuevaOrdenRepository.save(nuevaOrdenModel);
  }


  public async modificarOrden(id: any, informacionNueva: EditarOrdenCreateDto): Promise<any> {
    let analisisIds = [];
    ...
    try {
      ...
      ordenAntigua.observacionesInternas = informacionNueva.observacionesInternas;
      ordenAntigua.numeroHistoriaClinica = informacionNueva.numeroHistoriaClinica;
      ordenAntigua.obraSocial = informacionNueva.obraSocial;
      ordenAntigua.solicitadoPor = informacionNueva.solicitadoPor;

      ordenAntigua.paciente = await this.pacienteRepository.findOne({ where: { id: informacionNueva.paciente.id } });
      ordenAntigua.medicos = await this.medicoRepository.findOne({ where: { id: informacionNueva.medicos.id } });
      ordenAntigua.laboratorista = await this.laboratoristaRepository.findOne({ where: { id: informacionNueva.laboratorista.id } });


      ordenAntigua.analisis = informacionNueva.analisis ? analisisIds.map(id => ({ id: Number(id) } as AnalisisEntity)) : [];
      ordenAntigua.grupos_analisis = informacionNueva.grupos_analisis ? gruposIds.map(id => ({ id: Number(id) } as GruposAnalisisEntity)) : [];
      const ordenActualizada = await this.nuevaOrdenRepository.save(ordenAntigua);
  ...
  public async actualizarEstadoOrden(id: number): Promise<any> {
    try {
      const orden = await this.nuevaOrdenRepository.findOne({ where: { id: id } });
